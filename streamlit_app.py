...
def display_colored_table_html(df, color_map, pretty_map, cell_color_map=None, title=None):
    """
    Generic HTML table:
     - df: DataFrame (values already formatted or raw)
     - color_map: dict mapping ORIGINAL metric keys (e.g. 'RPL_EJI') to header hex color
     - pretty_map: dict mapping ORIGINAL keys -> pretty column names (display)
     - cell_color_map: optional dict {row_index: {display_col_name: bg_color}}
    """
    if isinstance(df, pd.Series):
        df = df.to_frame().T

    # Work on a copy for display (rename columns to pretty display names)
    df_display = df.rename(columns=pretty_map)

    if title:
        st.markdown(f"### {title}")

    # Header HTML: map header colors by reverse-lookup from pretty_map
    header_html = "<tr>"
    for col in df_display.columns:
        # find original key for this display column
    ...
    # Body: format numeric floats to 3 decimals (no sign), leave strings as-is.
    body_html = ""
    for r_idx, row in df_display.iterrows():
        ...
        for col, val in row.items():
            ...
            if pd.isna(val):
                ...
                # If pre-formatted string (e.g. change table entries), show as-is
                cell_text = str(val)
...
def weaponized_arrows_of_truth(metrics, y1_values, y2_values):
    ...
    for i, metric in enumerate(metrics):
        ...
        color = "#ff0000" if diff > 0 else "lime"

        # Place label slightly offset from the bar center
        ...
        # Actual arrow (from Year1 tail -> Year2 head)
        annotations.append(dict(
            x=metric_name,        # head at category
            y=v2,                 # head y
            ax=metric_name,
            ay=v1,                # tail y
            xref="x",
...
# ------------------------------
# Plot function (vertical grouped bars)
# ------------------------------
...
def format_change_value(val):
    """Return string like '+0.012 ↑' or '-0.012 ↓' or 'No Data'"""
    if pd.isna(val):
        return "No Data"
    if isinstance(val, (int, float, np.number)):
        sign_val = f"{val:+.3f}"
        arrow = "↑" if val > 0 else ("↓" if val < 0 else "→")
        return f"{sign_val} {arrow}"
    return str(val)
...
if selected_parameter == "County":
    ...
    if subset1.empty or subset2.empty:
        ...
        # ---------------- Year Table ----------------
        # Build table with first column = location name, rows = years (raw numeric values)
        ...
        # Build a color map including first column (default white) - keys are ORIGINAL metric keys
        full_color_map = {location1_name: "#FFFFFF"}  # first column
        full_color_map.update(dataset_year1_rainbows)  # metrics

        # Build a pretty map including first column (original key -> display name)
        ...
        change_df = pd.DataFrame([change_values])  # one-row df with ORIGINAL metric keys

        # Column headers = rainbow colors (dataset_year1)
        change_header_colors = {m: dataset_year1_rainbows[m] for m in metrics}

        # Pretty names for change columns
        change_pretty = {m: f"Δ {pretty[m]}" for m in metrics}

        # Cell background colors (increase = light red, decrease = light green)
        # keys for cell_color_map must match display column names (i.e., change_pretty values)
        cell_colors = {
            0: {
                change_pretty[m]: ("#ffcccc" if change_values[m] > 0 else ("#ccffcc" if change_values[m] < 0 else "#ffffff"))
                for m in metrics
            }
        }

        # Build pretty df and format values (strings: +0.123 ↑)
        change_df_pretty = change_df.rename(columns=change_pretty)
        change_df_formatted = change_df_pretty.applymap(format_change_value)
    ...
    if nm_row1.empty or nm_row2.empty:
        ...
        full_color_map = {location1_name: "#FFFFFF"}
        full_color_map.update(dataset_year1_rainbows)
        full_pretty_map = {location1_name: location1_name, **pretty}
        ...
        # ---------------- Change Table ----------------
        change_values = compute_change_row(y1_values, y2_values, metrics)
        change_df = pd.DataFrame([change_values])
        change_header_colors = {m: dataset_year1_rainbows[m] for m in metrics}
        change_pretty = {m: f"Δ {pretty[m]}" for m in metrics}
        cell_colors = {
            0: {
